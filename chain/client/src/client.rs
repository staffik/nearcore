...
impl Client {
    ...
    pub(crate) fn update_validator_signer(&self, signer: InMemorySigner) {
        let validator_signer = Some(Arc::new(ValidatorSigner::InMemoryValidatorSigner(
            InMemoryValidatorSigner::from_signer(signer)
        )));
        self.validator_signer.update(validator_signer);
    }
}
